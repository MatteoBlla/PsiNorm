...
We first simulate a scRNA-seq experiment with four known clusters using $splatter$ bioC package.  Then in the unsupervised approach, we i) reduce dimentionality using PCA, ii) identify clusters using $clara$ partitional method and then we iii)  computed the Adjusted Rand Index (ARI) to compare the known and the estimated partition.

In the supervised approach, we i) reduce dimentionality using PCA, and we  ii)  computed the silhouette index of the  known partition in the reduced dimensional space. 

# Data Simulation

We simulate a matrix of counts with 2000 cellular samples and 10000 genes with splatter BioC package:
...
To visualize the data we used the first two Principal Components estimated starting from the raw count matrix.  
```{r}
set.seed(1234)
sce<-runPCA(sce, exprs_values="counts", scale=T)
df<-data.frame(PC1=reducedDim(sce)[,1],
               PC2=reducedDim(sce)[,2],
               group=sce$Group)
...
To normalize the raw count we used  PsiNorm normalization (pareto_norm function) and we visualized the data using the first two PCs. 

```{r}
sce<-PsiNorm::PsiNorm(sce, whichAssay="counts") 
#assay(sce, "PsiNorm")<-log2(t(t(counts(sce))/sizeFactors(sce))+1)
assay(sce, "PsiNorm")<-logcounts(logNormCounts(sce,
                                     center.size.factors=F))
set.seed(1234)
sce<-runPCA(sce, exprs_values="PsiNorm", scale=T)
df<-data.frame(PC1=reducedDim(sce)[,1],
               PC2=reducedDim(sce)[,2],
               group=sce$Group)
ggplot(df,aes(PC1, PC2, color=group))+
...
```{r}
sce<-scater::runPCA(sce, exprs_values="counts", scale=T)
groups<-cluster::clara(reducedDim(sce), k=nlevels(sce$Group))
a<-paste("ARI from raw counts:", 
         round(mclust::adjustedRandIndex(groups$clustering, sce$Group), 
               digits = 3))

sce<-scater::runPCA(sce, exprs_values="PsiNorm", scale=T)
groups<-cluster::clara(reducedDim(sce), k=nlevels(sce$Group))
b<-paste("ARI from PsiNorm normalized data:",
         round(mclust::adjustedRandIndex(groups$clustering, sce$Group), 
               digits = 3))

kableExtra::kable(rbind(a,b), row.names = F)
```
Pareto normalization considerably increases the ARI index.

## Supervised approach: Silhouette index

We calculate Silhouette index of both raw counts and PsiNorm normalized counts after PC dimension reduction exploiting known simulated clusters; higher the Silhouette, better the normalization. 
...
PCAp<-reducedDim(runPCA(sce, exprs_values="PsiNorm", scale=T), "PCA")
depth<-apply(counts(sce), 2, sum)
a<-paste("The Correlation from the raw data is:",
            round(abs(max(cor(PCA[,1], depth), cor(PCA[,2], depth))), digits=3))
b<-paste("The Correlation from the PsiNorm normalized data is:",
