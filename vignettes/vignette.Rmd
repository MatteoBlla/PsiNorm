...
author: 
    name: Matteo Borella and Davide Risso
    affiliation: Department of Biology and Department of Statistical Sciences, University of Padova 
date: "Last modified: April 7, 2021; Compiled: `r format(Sys.time(), '%B %d, %Y')`"
vignette: >
...
BiocManager::install("MatteoBlla/PsiNorm")
```
...
In the supervised approach, we i) reduce dimentionality using tSNE, and we  ii)  compute the silhouette index of the  known partition in the reduced dimensional space. 
...
```{r}
set.seed(1234)
sce<-runTSNE(sce, exprs_values="counts", scale=TRUE)
...
To normalize the raw count we used the PsiNorm normalization and we visualized the data using the first two tSNE components. 
...
```{r}
set.seed(1234)
sce<-runTSNE(sce, exprs_values="logcounts", scale=TRUE)
df<-data.frame(TSNE1=reducedDim(sce)[,1],
               TSNE2=reducedDim(sce)[,2],
               group=sce$Group)
ggplot(df,aes(TSNE1, TSNE2, color=group))+
...
```{r}
set.seed(4444)
PCA<-reducedDim(runPCA(sce, exprs_values="counts", scale=TRUE), "PCA") 
PCAp<-reducedDim(runPCA(sce, exprs_values="logcounts", scale=TRUE), "PCA")
